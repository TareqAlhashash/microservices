package com.investorbook.paymentservice.dao.entities;
...
/**
 * Idempotency record for OrderPlaced: this service has no other state to
 * guard on (unlike order-service's own status field), so a dedicated
 ...
 * it - inserting a duplicate id fails, which PaymentEventListener treats
 * as "already processed, skip" rather than an error.
 *
