...
Six independently deployable Maven modules (five original services plus `order-service`, the
first of a new event-driven purchase flow — see "Event-driven purchase flow" below) plus one
shared library, each with its own `pom.xml`. There is no parent/reactor POM — modules are built
and run separately, and **`member-service`, `auth-service`, `resource-service`, `api-gateway`,
`common`, and `order-service` have all been upgraded and given test coverage** (see below); only
`eureka-server` is still exactly as originally generated.
...
2. PostgreSQL reachable at `jdbc:postgresql://localhost/investorbook` (user `postgres` / pass `pass`)
   — required by `auth-service`, `member-service`, and `order-service`
...
7. Kafka reachable at `localhost:9092` (`docker compose up -d` at the repo root) —
   required by `order-service` (port **8200**), see "Event-driven purchase flow" below
...
to Kafka. From there, the (planned) chain is choreographed, not orchestrated - no central saga
